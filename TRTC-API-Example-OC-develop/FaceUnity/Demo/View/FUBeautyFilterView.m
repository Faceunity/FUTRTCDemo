...
- (void)configureUI {
    ...
    if (!self.filterSlider.hidden) {
        self.filterSlider.value = [self.viewModel filterValueAtIndex:self.viewModel.selectedIndex];
    }
}
...
- (__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.imageView.image = [self.viewModel filterIconAtIndex:indexPath.item];
    cell.textLabel.text = [self.viewModel filterNameAtIndex:indexPath.item];
    return cell;
}
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (!self.filterSlider.hidden) {
        self.filterSlider.value = [self.viewModel filterValueAtIndex:indexPath.item];
    }
}
...
- (FUSlider *)filterSlider {
    if (!_filterSlider) {
        _filterSlider = [[FUSlider alloc] initWithFrame:CGRectMake(56, 15, CGRectGetWidth(self.frame) - 112, 30)];
